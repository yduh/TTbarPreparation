...
def systext():
	...
	arg = {'mt':'top mass','pdf':'PDFs','pdf1':'PDFs','pdf2':'PDFs','alpha':'alpha_s in PDFs','rsfs':'fact. & reno. scales','hdamp':'NLO shower matching','isr':'initial state radiation','fsr':'final state radiation','tune':'underlying events','bfrag':'b-jet fragmentation','bdecay':'B meson decaying Br.','color':'color reconnection'}
	#lx4.DrawLatex(0.3, 0.95, sys.argv[1]+', '+arg[sys.argv[2]])
	lx4.DrawLatex(0.35, 0.88, sys.argv[1]+', '+sys.argv[2])
...
h_st = createH(f, 'st')
hup_st = createHUp(f, 'st', sys.argv[2], ROOT.kOrange-3)
hdw_st = createHDw(f, 'st', sys.argv[2], ROOT.kCyan+1)
h_vj = createH(f, 'vj')
hup_vj = createHUp(f, 'vj', sys.argv[2], ROOT.kOrange-3)
hdw_vj = createHDw(f, 'vj', sys.argv[2], ROOT.kCyan+1)
h_qcd = createH(f, 'qcd')
...
for ibin in range(h.GetXaxis().GetNbins()):
	...
	h_st.GetXaxis().SetBinLabel(ibin+1,'%s-%s' %(mttlow,mtthigh))
	hup_st.GetXaxis().SetBinLabel(ibin+1,'%s-%s' %(mttlow,mtthigh))
	hdw_st.GetXaxis().SetBinLabel(ibin+1,'%s-%s' %(mttlow,mtthigh))
	h_vj.GetXaxis().SetBinLabel(ibin+1,'%s-%s' %(mttlow,mtthigh))
	hup_vj.GetXaxis().SetBinLabel(ibin+1,'%s-%s' %(mttlow,mtthigh))
	hdw_vj.GetXaxis().SetBinLabel(ibin+1,'%s-%s' %(mttlow,mtthigh))
	h.GetXaxis().LabelsOption('v')
	hup.GetXaxis().LabelsOption('v')
	hdw.GetXaxis().LabelsOption('v')
	h_st.GetXaxis().LabelsOption('v')
	hup_st.GetXaxis().LabelsOption('v')
	hdw_st.GetXaxis().LabelsOption('v')
	h_vj.GetXaxis().LabelsOption('v')
	hup_vj.GetXaxis().LabelsOption('v')
	hdw_vj.GetXaxis().LabelsOption('v')

hrup = createRatio(hup, h, ROOT.kRed, 't#bar{t}')
hrdw = createRatio(hdw, h, ROOT.kBlue, 't#bar{t}')
hrup_st = createRatio(hup_st, h_st, ROOT.kRed-7, 'single t')
hrdw_st = createRatio(hdw_st, h_st, ROOT.kBlue-7, 'single t')
hrup_vj = createRatio(hup_vj, h_vj, ROOT.kRed-7, 'V+jets')
hrdw_vj = createRatio(hdw_vj, h_vj, ROOT.kBlue-7, 'V+jets')
c, pad1, pad2, pad3, pad4 = createCanvasPads4()
...
h_st.Draw("same")
#hup_st.SetLineColor(ROOT.kOrange-3)
#hup_st.Draw("same")
#hdw_st.SetLineColor(ROOT.kCyan+1)
#hdw_st.Draw("same")
...
hrdw.Draw("psame")
pad3.cd()
hrup_st.GetYaxis().SetRangeUser(sysRangeDown, sysRangeUp)
hrup_st.Draw("p")
hrdw_st.Draw("psame")
pad4.cd()
hrup_vj.GetYaxis().SetRangeUser(sysRangeDown, sysRangeUp)
hrup_vj.Draw("p")
hrdw_vj.Draw("psame")
...
#linedely1_pad3.SetLineColor(30)
#linedely2_pad3.SetLineColor(30)
...
#linedely1_pad4.SetLineColor(30)
#linedely2_pad4.SetLineColor(30)
